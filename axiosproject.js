...
function refresh(){
    axios.get('https://crudcrud.com/api/62d890f5292446c0a507068940b75dba/addexpensive')
    .then((response)=>{
...
async function saveToLocalStorage(e){
    ...
    axios.post('https://crudcrud.com/api/62d890f5292446c0a507068940b75dba/addexpensive',expense)
    .then(()=>{
...
function ShowNewUsersOnScreen(user){
    ...
    const childHTML=`<li id=${user._id}> ${user.chooseexpensive}-${user.choosetheater}-${user.choosemovie}
                        <button onclick=deleteuser('${user._id}')> Delete user </button>
                        <button onclick=editUserDetails('${user.choosetheater}','${user.chooseexpensive}','${user.choosemovie}','${user._id}')>Edit</button></li>`
    itemlist.innerHTML=itemlist.innerHTML+childHTML;
}
function deleteuser(userid){
    axios.delete(`https://crudcrud.com/api/62d890f5292446c0a507068940b75dba/addexpensive/${userid}`)
    .then(() => {
